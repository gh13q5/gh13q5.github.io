...
3. DHCP 서버 구성 설정  
> 만약 분배할 주소 범위가 192.168.106.1~254라면, 254는 192.168.106.254가 된다.
{: .prompt-tip }
4. 서브넷 마스크 비트 입력  
5. 제외할 주소 범위 입력  
6. 임대기간 입력  
...
> 200.115.100.0/24에서 24가 마스크가 된다.
{: .prompt-tip } 
...
> 최소 사용은 최소 암호 사용 기간, 최대는 최대 암호 사용 기간을 뜻한다.
{: .prompt-tip }
3. `계정 정책` - `계정 잠금 정책` 클릭  
